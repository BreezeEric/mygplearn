...
def _protected_division(x1, x2):
    ...
    with np.errstate(divide='ignore', invalid='ignore'):
        return np.nan_to_num(np.where(np.abs(x2) == 0.0, np.divide(x1, x2), np.nan))


def _protected_sqrt(x1):
    """Closure of square root for negative arguments."""
    with np.errstate(divide='ignore', invalid='ignore'):
        return np.nan_to_num(np.where(np.sign(x1) >= 0, np.sqrt(x1), np.sign(x1)*np.sqrt(np.abs(x1))))


def _protected_log(x1):
    """Closure of log for zero arguments."""
    with np.errstate(divide='ignore', invalid='ignore'):
        return np.nan_to_num(np.where(np.sign(x1) >= 0, np.log(x1), np.sign(x1)*np.log(np.abs(x1))))


def _protected_inverse(x1):
    """Closure of inverse for zero arguments."""
    #with np.errstate(divide='ignore', invalid='ignore'):
    return np.nan_to_num(np.divide(1.0, x1))
...
def _rank(x1):
    x1 = pd.DataFrame(x1)
    return np.nan_to_num(x1.rank(pct=True).values)

def _scale(x1, a=1):
    a = int(a)
    return np.nan_to_num((a*x1 / np.nansum(np.abs(x1))))

def _signedpower(x1, a=2):
    a = int(a)
    return np.nan_to_num(np.sign(x1) * np.power(np.abs(x1), a))

def _delay(x1,d=5):
    x1 = pd.DataFrame(x1)
    d = 2 if int(d)<=0 else int(d)
    return np.nan_to_num(x1.shift(periods=d,axis=1).values.astype(float))
...
def _covariance(x1,x2,d=5):
    ...
    return np.nan_to_num(cov.astype(float)) 
...
def _correlation(x1,x2,d=5):
    ...
    return np.nan_to_num(corr.astype(float)) 

def _delta(x1,d=5):
    d = 2 if int(d)<=0 else int(d)
    return np.nan_to_num(x1.astype(float)-pd.DataFrame(x1).shift(periods=d,axis=1).values.astype(float))
...
def _ts_stddev(X, d=5):
    ...
    return np.nan_to_num(res.values.astype(float))
...
def _ts_sma(x1,n,m):
    ...
    return np.nan_to_num(x2.ewm(axis=1,alpha=m/n).mean().values)
...
def _ts_wma(x1,d=5):
    ...
    return np.nan_to_num(x2.astype(float))
...
def _ifcondition_g(condition_var1, condition_var2, x1, x2):
    ...
    A[flag] = x1
    A[~flag] = x2
    return np.nan_to_num(A.values.astype(float))
...
def _ts_sumif(x1, condition_var1, condition_var2, d=5):
    ...
    A[flag] = x1
    A[~flag] = 0
    return np.nan_to_num(A.rolling(axis=1,window=d).sum().values.astype(float))
...
def _ts_count(condition_var1, condition_var2, d=5):
    ...
    return np.nan_to_num(condition.rolling(window=d,axis=1).sum().values.astype(float))

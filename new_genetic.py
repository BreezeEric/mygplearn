...
from new_program import _Program
from new_fitness import _fitness_map, _Fitness
from new_functions import _function_map, _Function, sig1 as sigmoid
from new_utils import _partition_estimators
...
def _parallel_evolve(n_programs, parents, X, y, sample_weight, seeds, params,X_test=None,y_test=None):
...
#     if count:
#         if metric.greater_is_better:
#             retain_index=list(pd.Series(fitness).sort_values(ascending=False).index)[0]
#             programs.append(parents[retain_index])
#         else:
#             retain_index=list(pd.Series(fitness).sort_values(ascending=True).index)[0]
#             programs.append(parents[retain_index])
    if count:  
        if metric.greater_is_better:
    ...
    #         oob_sample_weight[indices] = 0
            ...
            program.raw_fitness_ = program.raw_fitness(X, y,X_test,y_test)
            if max_samples < n_samples:
                # Calculate OOB fitness
                #program.oob_fitness_ = program.raw_fitness(X, y, oob_sample_weight)
                program.oob_fitness_ = program.raw_fitness(X, y,X_test,y_test)
            
            if len(programs):
                program_str_list = [ii.__str__() for ii in programs]
                if program.__str__() not in program_str_list or while_count>=200:
...
#             program_str_list = [ii.__str__() for ii in programs]
#             if program.__str__() in program_str_list:
#                 continue
#             else:
#                 programs.append(program)
#                 break
...
class BaseSymbolic(BaseEstimator, metaclass=ABCMeta):
    ...
    def __init__(self,
        ...
        self.pre_test_fitness = 0
        self.test_fitness = 0
        self.pre_stop_count = 0
    ...
    def _verbose_reporter(self, run_details=None):
        ...
        if run_details is None:
            print('    |{:^25}|{:^42}|'.format('Population Average',
                                               'Best Individual'))
            print('-' * 4 + ' ' + '-' * 25 + ' ' + '-' * 42 + ' ' + '-' * 10)
            line_format = '{:>4} {:>8} {:>16} {:>8} {:>16} {:>16} {:>16} {:>16} {:>10}'
            print(line_format.format('Gen', 'Length', 'Fitness', 'Length',
                                     'Fitness', 
                                     #'len_program','len_set_program',
                                     'average_raw_fitness','best_raw_fitness',
            ...
            oob_fitness = 'N/A'
            line_format = '{:4d} {:8.2f} {:16g} {:8d} {:16g} {:>16} {:>16} {:>16} {:>10}'
            if self.max_samples < 1.0:
                oob_fitness = run_details['best_oob_fitness'][-1]
                line_format = '{:4d} {:8.2f} {:16g} {:8d} {:16g} {:16g} {:16g} {:16g} {:>10}'
            ...
            print(line_format.format(run_details['generation'][-1],
                                     ...
                                     run_details['average_raw_fitness'][-1],
                                     run_details['best_raw_fitness'][-1],
#                                      run_details['len_program'][-1],
#                                      run_details['len_set_program'][-1],
                                     oob_fitness,
                                     remaining_time))

    def fit(self, X, y, X_test = None,y_test = None,sample_weight=None,us=None):
        ...
        if isinstance(self.metric, _Fitness):
            ...
            if self.metric not in ('mean absolute error', 'mse', 'rmse',
                                   'pearson', 'spearman','pearson1','evaluation'):
...
#                                  'len_set_program':[]
        ...
        if self.warm_start:
            ...
            K = 2
            n_jobs, n_programs, starts = _partition_estimators(self.population_size*K, self.n_jobs)
            seeds = random_state.randint(MAX_INT, size=self.population_size*K)
            ...
            #从K倍的初始样本中选择1倍的个数，传给us
            icir_func = _fitness_map['pearson1']
            fitness_tempt = [icir_func(y,program.execute(X)) for program in population]
            pop_index = np.argsort(fitness_tempt)[::-1][:int(self.population_size)]
            initial_programs = [population[i] for i in pop_index]
        ...
        for gen in range(prior_generations, self.generations):
            ...
            fitness = list(np.nan_to_num(np.array(fitness)))

            if self._metric.greater_is_better:
                
                fitness_p = (np.exp(fitness-np.min(fitness)))/np.sum(np.exp(fitness-np.min(fitness)))
                population = list(np.random.choice(population,int(self.hall_of_fame),replace=False,p = fitness_p))
            else:
                
                fitness_p = (np.exp(fitness - np.max(fitness)))/np.sum(np.exp(fitness-np.max(fitness)))
                population = list(np.random.choice(population,int(self.hall_of_fame),replace=False,p = fitness_p))
                
                
            if parents is None:
                y_programs_pred = [np.zeros(np.array(y).shape)]
            else:
                if len(parents)>int(self.hall_of_fame*0.5):
                    parents_fitness = [program.fitness_ for program in parents]
                    parents_fitness = list(np.nan_to_num(np.array(parents_fitness)))
                    for i in range(len(parents_fitness)):
                        if parents_fitness[i] is None:
                            parents_fitness[i] = 0
                    
                  
                    if self._metric.greater_is_better:

                        fitness_p = (np.exp(parents_fitness-np.min(parents_fitness)))/np.sum(np.exp(parents_fitness-np.min(parents_fitness)))
                        pop_data = list(np.random.choice(parents,int(self.hall_of_fame*0.5),replace=False,p = fitness_p))
                        y_programs_pred = [da.execute(X) for da in pop_data]
                    else:

                        fitness_p = (np.exp(parents_fitness-np.max(parents_fitness)))/np.sum(np.exp(parents_fitness-np.max(parents_fitness)))
                        pop_data = list(np.random.choice(parents,int(self.hall_of_fame*0.5),replace=False,p = fitness_p))
                        y_programs_pred = [da.execute(X) for da in pop_data]
                else:
                    y_programs_pred = [program.execute(X) for program in parents]
            ...
            #对fitness增加因子相关性惩罚
            def population_pca(y_programs_pred):
                model = PCA(1)
                tempt_y = np.nan_to_num(np.array(y_programs_pred))
                tempt_y[np.isinf(tempt_y)] = 0
                tempt_y[np.abs(tempt_y)>=1E30] = 0
                s_pop,s_stock,s_date = tempt_y.shape
                if s_pop==1:
                    return np.nan_to_num(y_programs_pred[0])
                tempt0 = np.nan_to_num(tempt_y[:,:,0])
                tempt0[np.isinf(tempt0)] = 0
                tempt0[np.isnan(tempt0)] = 0
                tempt0[np.abs(tempt0)>=1E30] = 0
                res = model.fit_transform(tempt0.T)
                for i in range(1,s_date):
                    model = PCA(1)
                    tempt = np.nan_to_num(tempt_y[:,:,i])
                    tempt[np.isinf(tempt)] = 0
                    tempt[np.isnan(tempt)] = 0
                    tempt[np.abs(tempt)>=1E30] = 0
                    tempt_res = model.fit_transform(tempt.T)
                    res = np.hstack((res,tempt_res))
                return np.nan_to_num(res)
            
            pca1 = population_pca(y_programs_pred)
            
            for program in population:
                #program.fitness_ = program.fitness_coef(X,y,pca1,self._metric.greater_is_better)
                coef_tempt = program.coef(X,y,pca1)*program.metric.sign
                val = np.abs(program.execute(X))
                m,n = val.shape
                cc = 0
                for mm in range(m):
                    for nn in range(n):
                ...
                if np.abs(coef_tempt)>0.8:
                    ...
                    if self._metric.greater_is_better:
                        program.fitness_ = program.fitness(parsimony_coefficient) - 100*np.abs(coef_tempt)
                    else:
                        program.fitness_ = program.fitness(parsimony_coefficient) + 100*np.abs(coef_tempt)
                else:
                    if self._metric.greater_is_better:
                        program.fitness_ = program.fitness(parsimony_coefficient) - np.abs(coef_tempt)
                    else:
                        program.fitness_ = program.fitness(parsimony_coefficient) + np.abs(coef_tempt)
            ...
            r_fitness = [program.raw_fitness_ for program in population]
            #obb_fitness_list = [program.raw_fitness_(X_test,y_test) for program in population]
            if self._metric.greater_is_better:
            ...
            self.run_details_['average_raw_fitness'].append(np.mean(r_fitness))
            self.run_details_['best_raw_fitness'].append(best_program.raw_fitness_)
#             self.run_details_['len_program'].append(len([i.__str__() for i in self._programs[-1]]))
#             self.run_details_['len_set_program'].append(len(list(set([i.__str__() for i in self._programs[-1]]))))
            oob_fitness = np.nan
            if self.max_samples < 1.0:
...
#             if self.run_details_['len_set_program'][-1]<=5:
#                 break
            
            self.test_fitness = best_program.raw_fitness(X_test, y_test)
            if np.abs(self.test_fitness - self.pre_test_fitness)<1e-3:
                self.pre_stop_count+=1
            else:
                self.pre_stop_count = 0
            self.pre_test_fitness = self.test_fitness
            
            if self.pre_stop_count>=3:
                break
